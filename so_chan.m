function [qrs_amp, qrs_ind] = so_chan(X, Fs)

%SO_CHAN Performs the So-Chan QRS detection algorithm.
%
%   [QRS_AMP, QRS_IND] = SO_CHAN(X, FS) performs the So-Chan
%   detection algorithm [1] that detects QRS occurrences and
%   outputs their amplitude and index (QRS_AMP and QRS_IND
%   respectively). The function takes an input signal X sam-
%   pled at FS. The algorithm used here is derived from the
%   FD52 variant.
%
%   [1] H. H. So and K. L. Chan, "Development of QRS detec-
%   tion method for real-time ambulatory cardiac monitor,"
%   Proc. 19th Annu. Int. Conf. IEEE Eng. Med. Biol. Soc. 
%   Magnificent Milestones Emerg. Oppor. Med. Eng. (Cat. 
%   No.97CH36136), vol. 1, no. C, pp. 0, 1997.
%
%   Last revision in 24/01/2017.
%   V1.0: Performs the So-Chan algorithm, the FD52 variant.
%          Multiple detections are handled using an ignore
%          period, which corresponds to the physiological
%          minimum of RR interval. Testing with mitdb/100
%          gave 2274 detected R, with few misses due to
%          abnormally low amplitude.
%   V1.1: Corrected the derivative filter coefficients,
%          which were inverted. Also added plots to help
%          users understand the outputs better.
%
